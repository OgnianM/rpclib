 ...
 */
template <typename... Args> std::string pack_any(Args &&...any) {
  std::stringstream ss;
...
template <typename T> T unpack_single(const char *packed, size_t packed_size) {
  auto tmp = msgpack::unpack(packed, packed_size);
  T result;
...
template <typename socket_t> struct rpc_base {
    rpc_base(socket_t &&socket_) : socket(std::move(socket_)) {}
    rpc_base(const rpc_base &) = delete;
    rpc_base(rpc_base &&) = default;

    /**
     * The sender specifies the number of bytes to receive,
     * The function fails if the bytes to receive are greater than the buffer_size
     */
    void async_read(std::function<void(const asio::error_code &ec, std::size_t size)> &&f) {
        async_read_bytecount([f = std::move(f), this]() {
            if (message_size == -1) { // -1 denotes an exception, read the exception
