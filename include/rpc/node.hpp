...
struct ISocket {
    ...
    virtual void async_write(const asio::const_buffer& buffer,
                             std::function<void(const asio::error_code&, std::size_t)> handler) = 0;
    virtual void async_read(const asio::mutable_buffer& buffer,
                            std::function<void(const asio::error_code&, std::size_t)> handler) = 0;
...
struct node {
    ...
    void send_command(const std::string& function, const std::string& args,
                      std::function<void()> result_handler = [] {});
        ...
        functions[name] = std::make_shared<std::function<void(uint32_t uid, std::string&)>>(
            [=, this](uint32_t uid, std::string& packed_args) -> void {
                ...
                auto params = std::apply([]<typename... Ts>(Ts&&... args) ->
                                         typename traits::parameter_tuple {
                                             return {args...};
                                         }, decayed_params);
                     ...
                     [this, arg_tuple = std::move(arg_tuple), promise = std::move(promise)] {
                         ...
                         try {
                             ...
                             if constexpr (!std::is_same_v<Ret, void>) {
                                 auto return_value = unpack_single<Ret>(
                                     result.return_value.c_str(), result.return_value.size());
                                 this->read_enqueued();
...
struct service_provider : detail::declare_ssl_context<socket_t> {
     ...
     */
    service_provider(asio::io_context& context,
                     const asio::ip::tcp::endpoint& ep) : ctx(context), acceptor(ctx, ep) {}
